...
**Behavior lives in `src/hooks/`, not in the pages.** The important ones:
...
- **API shapes worth memorizing**: `tokenize(): Token[]`, `parse(): Program`, `interpret(program, source): string[]` (an array of lines, not a string), `translate(program, target): string`.
